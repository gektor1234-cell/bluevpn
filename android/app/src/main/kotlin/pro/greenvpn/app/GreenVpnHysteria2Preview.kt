...
object GreenVpnHysteria2Preview {
    data class Snapshot(
    ...
    private const val CONTROLLER_CLASS = "pro.greenvpn.hysteria.Hysteria2Controller"

    fun isAvailable(context: Context): Boolean {
        if (!BuildConfig.GREENVPN_HYSTERIA2_PREVIEW_ENABLED) return false
        return try {
            invoke("isAvailable", arrayOf(Context::class.java), context) as? Boolean == true
        } catch (_: Throwable) {
            false
        }
    }
    ...
    fun validateConfig(configText: String): String {
        ...
        return invoke("validateConfig", arrayOf(String::class.java), configText)?.toString()
            ?: throw IllegalStateException("Hysteria2 validator returned no config")
    }
    ...
    private fun ensureAvailable() {
        if (!BuildConfig.GREENVPN_HYSTERIA2_PREVIEW_ENABLED) {
            throw IllegalStateException("Hysteria2 preview engine is not included in this build")
        }
