...
class DnsttDebugReceiver : BroadcastReceiver() {
    override fun onReceive(context: Context, intent: Intent) {
            ...
            try {
                ...
                when (command) {
                    "connect" -> {
                        ...
                        DnsttController.connect(context, config.readText(Charsets.UTF_8))
                    }
                    "disconnect" -> DnsttController.disconnect(context)
                    "kill_engine" -> require(DnsttVpnService.debugKillEngineForTest()) {
        ...
        }, "GreenVPN-dnstt-Debug-Control").start()
    }

    private fun writeResult(context: Context, command: String, commandError: String) {
        val snapshot = DnsttController.snapshot(context)
    ...
    companion object {
        const val CONFIG_NAME = "greenvpn-dnstt-debug.json"
        const val RESULT_NAME = "greenvpn-dnstt-debug-result.json"
    }
}
