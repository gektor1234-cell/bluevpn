...
#
# Usage:
#   powershell -NoProfile -ExecutionPolicy Bypass -File .\bluevpn_patch_all_v10.ps1
#   powershell -NoProfile -ExecutionPolicy Bypass -File .\bluevpn_patch_all_v10.ps1 -SkipDeploy
#   powershell -NoProfile -ExecutionPolicy Bypass -File .\bluevpn_patch_all_v10.ps1 -SkipBuild
#
param(
...
function Run-Step {
  ...
  # capture output (so we can detect 'flutter help' situations)
  $out = & $Exe @Args 2>&1
  if ($null -ne $out) { $out | Out-Host }

  $code = $LASTEXITCODE

  # Flutter sometimes prints help and exits with code 0 -> detect it
  if ($Exe -ieq "flutter" -and -not $AllowHelp) {
    $txt = ($out | Out-String)
    if ($txt -match "Usage:\s*flutter\s*<command>" -or $txt -match "Manage your Flutter app development\." ) {
      throw ("Flutter printed help instead of running a command. Check argument passing. Tried: flutter {0}" -f ($Args -join ' '))
    }
  }

  if ($code -ne 0) {
    throw ("Command failed (exit {0}): {1} {2}" -f $code, $Exe, ($Args -join ' '))
  }
}
