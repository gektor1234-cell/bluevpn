...
#
# Usage:
#   powershell -NoProfile -ExecutionPolicy Bypass -File .\bluevpn_patch_all_v11.ps1
#   powershell -NoProfile -ExecutionPolicy Bypass -File .\bluevpn_patch_all_v11.ps1 -SkipDeploy
#   powershell -NoProfile -ExecutionPolicy Bypass -File .\bluevpn_patch_all_v11.ps1 -SkipBuild
#
param(
...
function Run-Step {
  ...
  if ($Exe -ieq "flutter" -and -not $AllowHelp) {
    $txt = ($out | Out-String)
    if ($txt -match "Manage your Flutter app development\.") {
      $cmd = "$Exe"
      if ($Arguments.Count -gt 0) { $cmd = "$cmd $($Arguments -join ' ')" }
      throw ("Flutter printed help instead of running a command. Check argument passing. Tried: " + $cmd)
    }
  }
...
'''.replaceAll('__EP__', ep);
  ...
  Future<bool> _serviceExists() async {
    ...
    if (out.contains('not installed')) return false;
