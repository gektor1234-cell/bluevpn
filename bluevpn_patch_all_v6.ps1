...
Set-Content -Encoding UTF8 -Path $main -Value $script:text
Ok "Imports OK."
...
Set-Content -Encoding UTF8 -Path $main -Value $script:text2
Ok "BACKEND patched."

# ============================
# 4.5) Fix call sites: .managedConfigPath() -> .managedConfigPath
# ============================
Info "Fixing managedConfigPath() call sites..."
$srcText = Get-Content -Raw -Encoding UTF8 $main
$before = [regex]::Matches($srcText, '(?<=\.)managedConfigPath\(\)').Count
if ($before -gt 0) {
  $srcText = [regex]::Replace($srcText, '(?<=\.)managedConfigPath\(\)', 'managedConfigPath')
  Set-Content -Encoding UTF8 -Path $main -Value $srcText
}
$after = [regex]::Matches($srcText, '(?<=\.)managedConfigPath\(\)').Count
Ok "managedConfigPath() calls: $before -> $after"
