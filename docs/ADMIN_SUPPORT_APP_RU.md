...
- Закрытый адрес: `https://admin.greenvpn.pro/`.
- Внешний барьер: Nginx Basic Auth, `noindex`, запрет iframe.
- Основной вход: staff email/password, короткоживущая сессия, RBAC и 2FA-readiness.
- Backend `0.9.121-admin.1` работает на Timeweb и RUVDS Moscow; биллинг пишет только Timeweb.
- Админка единообразно управляет аккаунтами и устройствами Android/Windows.
- Большие списки используют серверную пагинацию и индексы вместо фиксированных первых 100 строк и N+1-запросов подписок.
- На обзор вынесены очередь внимания, Android/Windows, активность 24 часа/7 дней/30 дней, подписки, платежи, поддержка и инциденты.
- Есть глобальный поиск аккаунта, фильтры, сортировка, массовые действия до 100 аккаунтов и CSV-выгрузки пользователей, платежей, поддержки и аудита.
- Карточка аккаунта объединяет подписку, лимит устройств, Android/Windows-устройства, платежи, обращения и административные действия. Изменения требуют причины и пишутся в аудит.
- CSV защищён от spreadsheet formula injection и не содержит raw provider payment method ids.
- Проверено: 98 backend-тестов, Python/JavaScript syntax, 264 уникальных HTML id, desktop/mobile rendering и live smoke обоих control-plane.
