...
- Five-stage preview checkpoint: `C:\Users\gekto\GreenVPN_Checkpoints\five_stage_transport_preview_20260712`; the exact verified commit is recorded in `checkpoint.json` inside the checkpoint.
...
- NL2 services for all five canaries are active. dnstt now sits behind isolated dnsdist `2.1.0` on public UDP+TCP/53; the dnstt daemon itself is loopback-only on UDP/5353. The frontend authoritatively serves NS/SOA, refuses unrelated zones, runs with `healthCheckMode="up"`, `TasksMax=8192` and `LimitNOFILE=16384`. Cloudflare resolves both NS and SOA. Full readiness passed with `server_data_plane_ready=true`, `doh_delegation_ready=true`, stable transports active and no secrets printed.
...
- Android dnstt physical proof passed on Samsung SM-A226B: NL2 egress `5.129.216.42`, production and both paid-beta APIs `200`, YouTube `204`, one child process with bidirectional traffic, watchdog fail-closed cleanup, reconnect, final `down`, zero remaining processes/services and plaintext/runtime cleanup. Report SHA-256: `1D4B1F8A3350CA3CD6FA0DA25A1967A3AFE265A3586E2DD7A9EA3A4A9D9562C4`.
- Reusable new-node procedure is `docs/SERVER_SECURITY_CONTOUR_INTEGRATION_RUNBOOK_RU.md`. It parameterizes provider/server input, stable fingerprints, guarded bootstrap/readiness/rollback, safe SSH transport, physical gates and checkpoint creation. It does not authorize deployment to another server.
