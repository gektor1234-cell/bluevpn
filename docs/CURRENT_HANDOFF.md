...
- Windows AWG2 transport preview remains isolated from stable. Physical narrow-route engine proof passed with a fresh handshake and exact restoration of `device20_full`; the first full-tunnel run exposed endpoint-route recursion and was correctly rejected. Security review then found that the original preview `LocalSystem` service was installed under user-writable `%LOCALAPPDATA%`; do not use that installed instance. The rebuilt artifact installs under protected `%ProgramFiles%`, removes the legacy path and broad ProgramData ACLs, makes the task fail closed on broad write/reparse points, and includes the guarded `/32` endpoint route fix. Artifact SHA-256 is `FB015C8BDE23FB7B03C2D361DC7DF92B63D9A97D7FDBD57578B47325FDF5C689`; final proof requires one UAC-confirmed reinstall and full-tunnel smoke.
