...
## Дополнение: Windows 0.3.20, быстрый поиск подключения

Этот раздел новее остальных Windows-значений в документе. Windows production
`0.3.20+2921` и paid-beta `0.3.20-paid-beta.1+2921` опубликованы как optional
через Timeweb и RUVDS из чистого source anchor
`790c4b66aa9eb1dacaecca388d4dba93185e28a9`.

Исходная задержка `0.3.19` составляла `93.7-96.3` секунды. Конфиг и каталог
занимали только около `5-7` секунд; основное время уходило на ожидание
WireGuard, повторные тяжёлые диагностики и последовательный YouTube probe.
Ожидание ограничено, повторная диагностика облегчена, probes распараллелены, а
последний рабочий маршрут запоминается на 24 часа.

Отдельно исправлен пропуск уже активного внешнего VPN: PowerShell, запущенный
через shell, терял вывод списка служб. Прямой запуск `powershell.exe` теперь
корректно обнаруживает конфликт. Точная установленная `0.3.20+2921` завершила
этот сценарий за `11.277` секунды после одного кандидата, не запустила следующий
маршрут, оставила Green VPN выключенным и сохранила внешний AmneziaWG. Источник
доказательства — первичный журнал приложения. Четыре запуска общего UI
smoke-wrapper не распознали локализованный результат, хотя cleanup прошёл; их
JSON нельзя считать успешными physical reports.

Чистый успешный путь был физически проверен на непосредственно предыдущем
кандидате с тем же connect-path за примерно `9.9` секунды, включая YouTube
`204`. Точный clean-path `2921` автоматически не повторялся: внешний Amnezia
работает с более высоким Windows integrity и не может быть приостановлен
неповышенным процессом Codex. После этого smoke продуктовый diff ограничен
обнаружением конкурирующего VPN и ранней остановкой каскада.
...
Оба EXE остаются `NotSigned` по принятому владельцем решению.

Проверки: Flutter analyze, `68` passed / `6` skipped, release gate
`0` warnings / `0` errors, package audits, public bodies `8/8`, public surface
`31/31`. Rollback:
