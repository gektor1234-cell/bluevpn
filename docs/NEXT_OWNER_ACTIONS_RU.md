...
Последнее обновление: 2026-08-31 МСК

> Текущий платёжный маршрут: YooKassa плюс ручные чеки НПД. Production status
> зафиксирован в `PAYMENTS_RU.md`; старые разделы Prodamus/Robokassa ниже
...
| 3 | Принять legal/tax/KYC решения для продаж | **Готово для текущего ручного НПД:** YooKassa используется без провайдерского чека, официальный чек создаётся и аннулируется через «Мой налог» |
| 4 | Разрешить реальные денежные операции | **Готово для текущего релиза:** один owner-approved payment/refund smoke завершён; ручные продажи/возвраты включены на primary, автоматические списания выключены |
...
| 8 | Дать production go/no-go | **Готово:** Android `0.4.12` опубликован обязательным stable-обновлением; для будущей подписанной Windows, recurring или рекламной версии потребуется новое разрешение |
| 9 | Провести финальную пользовательскую приёмку | **Готово для Android 0.4.12:** exact physical install, UI, data-plane и network-lifecycle acceptance пройдены |

## Что блокирует формальное закрытие текущей v1

Инженерных и внешних сервисных блокеров у текущего direct-download релиза с
ручной оплатой нет. Android `0.4.12+2026083003` и Windows `0.4.6+4636`
опубликованы как mandatory stable на обоих control plane. Windows остаётся
честно `NotSigned`; принятый риск SmartScreen не блокирует direct-download.
...
Пункты 3 и 4 для ручных YooKassa/НПД продаж закрыты. Sales и refunds включены
только на primary; fallback read-only. Renewal charges остаются выключенными и
требуют отдельного будущего owner gate и нового smoke.
...
уже физически отправляются; автоматические списания и ограничивающие функции
...
внешний мониторинг и проверка обоих зеркал. Повторный денежный smoke не нужен,
пока не изменён provider, налоговый workflow или billing-код.

## Главное правило

- Не писать сюда реальные пароли, токены, SMTP password, `admin_token`, payment-provider secrets, SSH password и WireGuard private keys.
...
Android `0.4.8+2026082802` и Windows `0.4.6+4636` являются текущими
...
- SMTP/email-code production-ready; YooKassa + ручной НПД прошли полный
  payment/refund acceptance, ручные продажи работают на primary;
