...
7. **Тесты и smoke** - выполнено в изолированном тестовом контуре 2026-07-10.
   - Backend `0.9.106-paid-beta.5` работает отдельно на `127.0.0.1:8010` у Timeweb и RUVDS; current release `paid-beta-0.3.0-paid-beta.5-2026071005-r8`.
   ...
   - Тридцать один backend/DB-sync/package тест и release gate 0/0 проходят.
...
Техническая часть доступная без владельца завершена. Android real-device gate и полный Windows `.10` physical/recovery gate закрыты; `.10` опубликована только в beta-контуре. Billing работает в single-writer режиме Timeweb. YooKassa key перевыпущен и проверен на обоих control-plane, реальный платёж 149 RUB успешно активирован и синхронизирован на RUVDS, а fallback subscription/bootstrap прошёл при остановленном Timeweb beta. До генерации 20 кодов владелец должен подтвердить terms/privacy. Production остаётся замороженным.
