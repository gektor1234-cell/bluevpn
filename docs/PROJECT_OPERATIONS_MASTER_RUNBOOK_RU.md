...
- Четыре файла основной страницы устанавливаются guarded-скриптом
  `scripts/server/install_main_site_release.sh`. Архив не может содержать другие
  пути, symlink или downloads; при ошибке nginx/HTTPS восстанавливается root-only
  rollback-копия.
