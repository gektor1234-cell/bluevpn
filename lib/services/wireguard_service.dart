...
class WireGuardService {
  ...
  static Future<ExecResult> connect() async =>
      _runNet(['start', tunnelServiceName]);

  static Future<ExecResult> disconnect() async =>
      _runNet(['stop', tunnelServiceName]);

  static Future<ExecResult> dumpLogTail({int lines = 200}) async {
    final res = await Process.run(wireguardExe, [
      '/dumplog',
      '/tail',
    ], runInShell: true);
    ...
    final tail = split.length <= lines
        ? split
        : split.sublist(split.length - lines);
