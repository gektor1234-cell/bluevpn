...
if ask_yes_no "Configure public legal/requisites pages for YooKassa now?" "n"; then
...
if ask_yes_no "Generate/rotate one-time auth code pepper for email/phone login?" "y"; then
...
if ask_yes_no "Configure YooKassa production payments now?" "n"; then
  ...
  secret_key="$(read_secret "YOOKASSA_SECRET_KEY")"
  add_env "YOOKASSA_SHOP_ID" "${shop_id}"
  add_env "YOOKASSA_SECRET_KEY" "${secret_key}"
