...
changelog = json.dumps(
    [
        "Перед загрузкой обновления Green VPN отключается и отменяет автозапуск.",
        "После обновления или отмены загрузки VPN остаётся выключенным.",
        "Если в Android активен другой VPN, приложение попросит отключить его перед загрузкой.",
    ],
    ensure_ascii=False,
)
