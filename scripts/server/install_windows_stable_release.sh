...
python3 - \
  "$ENV_FILE" "$VERSION" "$DOWNLOAD_URL" "$SHA256" "$REQUIRED" "$released_at" <<'PY'
import os
import pathlib
import re
import sys

path_raw, version, download_url, sha256, required, released_at = sys.argv[1:]
...
updates = {
    ...
    "GREENVPN_UPDATE_CHANGELOG": (
        f"Green VPN {version}: быстрое подключение Windows; "
        "углубленная диагностика и резервные маршруты перенесены в фон."
    ),
...
python3 - \
  ...
  "$INSTALLER" "$REQUIRED" "$released_at" <<'PY'
import json
import pathlib
import sqlite3
import sys
...
changelog = json.dumps(
    [
        "Ускорено первое подключение на Windows.",
        "Проверки резервных маршрутов перенесены в фон.",
        "Сохранено автоматическое восстановление после реального сбоя.",
    ],
    ensure_ascii=False,
)
...
python3 - \
  "$VERSION" "$BUILD_NUMBER" "$SHA256" "$REQUIRED" \
  "$stable_manifest" "$public_product_manifest" <<'PY'
import json
import sys

version, build_number, sha256, required_raw, stable_raw, public_product_raw = sys.argv[1:]
