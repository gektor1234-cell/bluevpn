...
updates = {
    ...
    "GREENVPN_UPDATE_CHANGELOG": (
        f"Green VPN {version}: окно установщика автоматически закрывается "
...
changelog = json.dumps(
    [
        "Перед загрузкой обновления отключаются Green VPN и поддерживаемые сторонние VPN.",
        "Сторонний VPN больше не восстанавливается, в том числе при ошибке подключения Green VPN.",
        "После обновления или отмены загрузки VPN остаётся выключенным.",
    ],
    ensure_ascii=False,
)
