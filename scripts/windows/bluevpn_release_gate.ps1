...
$requiredBackendSafetyFragments = @(
    ...
    'def prodamus_signature(',
    'def prodamus_signature_valid(',
    'def claim_prodamus_link_create(',
    ...
    'def robokassa_payment_readiness(',
    'def robokassa_result_signature_valid(',
    'def sync_robokassa_billing_order(',
    'provider_create_attempted_at',
    'def claim_robokassa_invoice_create(',
    'payment_provider_reconciliation_pending',
    'def billing_writer_primary(',
    'def billing_callback_primary(',
...
$processRouterInstallerFragments = @(
    ...
    'B4759403D1550594A6032DA4869C6666B234B88868ED19D8A1FD38372B7349CE'
)
...
$transportPreviewRouteFragments = @(
    ...
    'Green VPN never restarts another VPN.',
...
$windowsRuntimeFailoverChecks = [ordered]@{
    ...
    'Windows privileged tasks commit a protected active mode marker' = @(
        ($vpnTaskScript + "`n" + $transportPreviewVpnTaskScript + "`n" + $transportSelectiveRoutingScript + "`n" + $transportSelectiveRoutingTestScript),
        ...
        'Global\GreenVPN.RuntimeMutation',
        'nestedCleanupGenerationReused',
        'registryTypeRetryPassed',
        'runtimeMutationMutexPassed',
        'invalidRegistryContractRejected',
        'nestedTransitionStartRejected',
        'abandonedGenerationAdopted',
        'RuntimeStateGeneration',
    ...
    'Windows emergency smoke recovery stops UI before disconnect' = @(
        ...
        'Stop-GreenVpnUi',
        'Install-ExternalVpnServiceForRecovery',
        '.conf.dpapi',
        '$services | Where-Object { $_.Name -notin $managedServiceNames }',
        'greenUiStopped',
