...
try {
    ...
    Invoke-Adb -Arguments @(
        'shell', 'run-as', $Package, 'rm', '-f', 'files/greenvpn-naive-permission-result.txt'
    ) | Out-Null
    Invoke-Adb -Arguments @(
        'shell', 'am', 'start',
    ...
    do {
        ...
        try {
            $ErrorActionPreference = 'Continue'
            $permissionResult = (@(
                & $Adb -s $Serial shell run-as $Package cat files/greenvpn-naive-permission-result.txt 2>$null
            ) -join '').Trim()
