param(
    ...
    [string]$ReportPath = 'C:\Users\gekto\GreenVPN_Checkpoints\android_vless_reality_preview_physical_20260712.json'
)
...
function Invoke-DebugCommand {
    ...
    Invoke-Adb -Arguments @('shell', 'run-as', $Package, 'rm', '-f', 'files/greenvpn-vless-debug-result.json') | Out-Null
    ...
    do {
        ...
        try {
            $ErrorActionPreference = 'Continue'
            $raw = (@(& $Adb -s $Serial shell run-as $Package cat files/greenvpn-vless-debug-result.json 2>$null) -join '').Trim()
...
function Get-ServiceRecordCount {
    return @(
        Invoke-Adb -Arguments @('shell', 'dumpsys', 'activity', 'services', $Package) |
            Select-String -Pattern '^\s*\* ServiceRecord\{.*VlessRealityVpnService'
    ).Count
}
...
function Test-RuntimeConfigRemoved {
    ...
    try {
        $ErrorActionPreference = 'Continue'
        $found = @(& $Adb -s $Serial shell run-as $Package find no_backup/vless-preview -maxdepth 1 -type f `
...
try {
    ...
    if ($preflight.state -ne 'down' -or (Get-EngineProcessCount) -ne 0 -or (Get-ServiceRecordCount) -ne 0) {
        throw 'Android VLESS REALITY preflight cleanup did not reach a clean down state.'
    }
    ...
    if (-not $report.connected -or $connected.state -ne 'up') {
        throw "VLESS REALITY did not reach up: $($connected.engineError) $($connected.commandError)"
    }
    if ($report.canaryEgress -ne $ExpectedEgress) {
        throw "Unexpected Android VLESS REALITY egress: $($report.canaryEgress); $($report.egressError)"
    }
    foreach ($key in @('egressStatus','productionApiStatus','paidBetaPrimaryStatus','paidBetaFallbackStatus')) {
        if ([int]$report[$key] -ne 200) {
            ...
            throw "Android VLESS REALITY probe failed: $key=$($report[$key]); $($report[$errorKey])"
        }
    }
    if ([int]$report.youtubeStatus -notin @(200, 204)) {
        throw "Android VLESS REALITY probe failed: youtubeStatus=$($report.youtubeStatus); $($report.youtubeError)"
    }
    if ($report.rxBytes -le 0 -or $report.txBytes -le 0 -or $report.processCountWhileUp -ne 1) {
        throw 'Android VLESS REALITY did not prove bidirectional traffic and one exact child process.'
    }
    ...
    if ($report.watchdogState -ne 'error' -or $report.watchdogProcessCount -ne 0 -or $report.watchdogServiceCount -ne 0) {
        throw "Android VLESS REALITY watchdog cleanup failed: state=$($report.watchdogState) process=$($report.watchdogProcessCount) service=$($report.watchdogServiceCount)"
    }
    ...
    if (-not $reconnected.connected -or $report.reconnectState -ne 'up' -or $report.reconnectEgress -ne $ExpectedEgress) {
        throw 'Android VLESS REALITY did not recover after fail-closed cleanup.'
    }
    ...
    if ($report.finalState -ne 'down' -or $report.finalProcessCount -ne 0 -or
        ...
        throw 'Android VLESS REALITY final cleanup did not reach a clean down state.'
    }
    $report.success = $true
} catch {
    ...
    try {
        $remaining = @(& $Adb -s $Serial shell run-as $Package ls files/greenvpn-vless-debug.json 2>$null)
...
Write-Output "Android VLESS REALITY preview physical smoke passed. Report: $ReportPath"
