...
typedef struct FLOW_PID_ENTRY {
    ...
    struct FLOW_PID_ENTRY *next;
} FLOW_PID_ENTRY;

#define FLOW_PID_BUCKETS 4096
...
static volatile LONG resolved_attribution_count = 0;
...
static int send_all(SOCKET sock, const char *buf, int len)
{
    ...
    while (sent < len) {
        ...
        if (n == SOCKET_ERROR) {
            ...
            if (err == WSAETIMEDOUT && running) continue;
            return SOCKET_ERROR;
        }
        sent += n;
    }
...
static int recv_exact(SOCKET sock, void *buffer, int len)
{
    char *cursor = (char *)buffer;
    int received = 0;
    while (received < len) {
        int n = recv(sock, cursor + received, len - received, 0);
        if (n == 0) return SOCKET_ERROR;
        if (n == SOCKET_ERROR) {
            int err = WSAGetLastError();
            if (err == WSAEINTR) continue;
...
static int socks5_read_reply(SOCKET sock, unsigned char *reply_code)
{
    unsigned char header[4];
    if (recv_exact(sock, header, sizeof(header)) != sizeof(header) ||
        header[0] != SOCKS5_VERSION)
        return -1;
    if (reply_code != NULL) *reply_code = header[1];
    ...
    if (header[3] == SOCKS5_ATYP_IPV4) {
        ...
        address_len = 16;
    } else if (header[3] == SOCKS5_ATYP_DOMAIN) {
        unsigned char length = 0;
        if (recv_exact(sock, &length, 1) != 1) return -1;
        address_len = length;
    } else {
        return -1;
    }
    ...
    if (address_len + 2 > (int)sizeof(discard)) return -1;
    return recv_exact(sock, discard, address_len + 2) == address_len + 2 ? 0 : -1;
}
...
static DWORD get_socket_port_pid_v4(BOOL is_udp, UINT16 local_port,
    UINT32 remote_ip, UINT16 remote_port, BOOL *used_cross_family);
...
static UINT32 flow_pid_hash(BOOL is_ipv6, BOOL is_udp, UINT16 local_port,
                            UINT16 remote_port, const void *local_addr,
                            const void *remote_addr)
{
    UINT32 hash = ((UINT32)local_port << 16) ^ remote_port ^
        (is_udp ? 0x9E3779B9u : 0x85EBCA6Bu) ^
        (is_ipv6 ? 0xC2B2AE35u : 0x27D4EB2Fu);
    const UINT8 *local = (const UINT8 *)local_addr;
    const UINT8 *remote = (const UINT8 *)remote_addr;
    size_t length = is_ipv6 ? 16 : 4;
    for (size_t i = 0; i < length; i++) {
        hash = (hash * 16777619u) ^ local[i];
        hash = (hash * 16777619u) ^ remote[i];
    }
...
static BOOL flow_pid_matches(const FLOW_PID_ENTRY *entry, BOOL is_ipv6,
                             ...
                             const void *remote_addr)
{
    if (entry->is_ipv6 != is_ipv6 || entry->is_udp != is_udp ||
        entry->local_port != local_port || entry->remote_port != remote_port)
        return FALSE;
    if (is_ipv6) {
        return memcmp(entry->local_ip6, local_addr, 16) == 0 &&
            memcmp(entry->remote_ip6, remote_addr, 16) == 0;
    }
    return entry->local_ip == *(const UINT32 *)local_addr &&
        entry->remote_ip == *(const UINT32 *)remote_addr;
}

static void store_flow_pid(BOOL is_ipv6, BOOL is_udp, UINT16 local_port,
                           UINT16 remote_port, const void *local_addr,
                           const void *remote_addr, DWORD pid)
{
    ...
    FLOW_PID_ENTRY *entry = flow_pid_table[bucket];
    while (entry != NULL) {
        if (flow_pid_matches(entry, is_ipv6, is_udp, local_port, remote_port,
                             local_addr, remote_addr)) {
            entry->pid = pid;
    ...
    entry = (FLOW_PID_ENTRY *)calloc(1, sizeof(FLOW_PID_ENTRY));
    if (entry != NULL) {
        ...
        entry->updated_at = GetTickCount64();
        if (is_ipv6) {
            memcpy(entry->local_ip6, local_addr, 16);
            memcpy(entry->remote_ip6, remote_addr, 16);
        } else {
            entry->local_ip = *(const UINT32 *)local_addr;
            entry->remote_ip = *(const UINT32 *)remote_addr;
        }
        entry->next = flow_pid_table[bucket];
        flow_pid_table[bucket] = entry;
    }
    ReleaseSRWLockExclusive(&flow_pid_lock);
}

static DWORD lookup_flow_pid(BOOL is_ipv6, BOOL is_udp, UINT16 local_port,
                             UINT16 remote_port, const void *local_addr,
                             const void *remote_addr)
{
    DWORD pid = 0;
    ...
    AcquireSRWLockShared(&flow_pid_lock);
    FLOW_PID_ENTRY *entry = flow_pid_table[bucket];
    while (entry != NULL) {
        if (flow_pid_matches(entry, is_ipv6, is_udp, local_port, remote_port,
                             local_addr, remote_addr)) {
            if (now - entry->updated_at <= FLOW_PID_TTL_MS)
                pid = entry->pid;
            break;
        }
        entry = entry->next;
    }
    ReleaseSRWLockShared(&flow_pid_lock);
    return pid;
}

static DWORD get_flow_pid_v4(UINT32 local_ip, UINT16 local_port,
                             UINT32 remote_ip, UINT16 remote_port, BOOL is_udp)
{
    return lookup_flow_pid(FALSE, is_udp, local_port, remote_port,
                           &local_ip, &remote_ip);
}

static DWORD get_flow_pid_v6(const UINT8 local_ip6[16], UINT16 local_port,
                             const UINT8 remote_ip6[16], UINT16 remote_port,
                             BOOL is_udp)
{
    return lookup_flow_pid(TRUE, is_udp, local_port, remote_port,
                                  ...
                                  UINT16 remote_port)
{
    ...
    socket_port_pid_table[index][local_port].remote_port = remote_port;
    socket_port_pid_table[index][local_port].has_remote =
        remote_addr != NULL && remote_port != 0;
    memset(socket_port_pid_table[index][local_port].remote_addr, 0,
           sizeof(socket_port_pid_table[index][local_port].remote_addr));
    if (socket_port_pid_table[index][local_port].has_remote)
    {
        memcpy(socket_port_pid_table[index][local_port].remote_addr,
               remote_addr, is_ipv6 ? 16 : sizeof(UINT32));
    }
    ReleaseSRWLockExclusive(&socket_port_pid_lock);
}

static BOOL socket_entry_is_fresh(const SOCKET_PORT_PID_ENTRY *entry,
                                  ULONGLONG now)
{
    return entry->pid != 0 && entry->has_remote &&
...
static DWORD select_socket_entry_pid(const SOCKET_PORT_PID_ENTRY *same,
                                     BOOL same_matches,
                                     const SOCKET_PORT_PID_ENTRY *cross,
                                     BOOL cross_matches,
                                     BOOL *used_cross_family)
{
    if (used_cross_family != NULL) *used_cross_family = FALSE;
    if (same_matches && cross_matches && same->pid != cross->pid)
        return 0;
    if (same_matches)
        return same->pid;
    if (cross_matches)
    {
        if (used_cross_family != NULL) *used_cross_family = TRUE;
        return cross->pid;
    }
    return 0;
}
...
{
    AcquireSRWLockExclusive(&flow_pid_lock);
    for (int i = 0; i < FLOW_PID_BUCKETS; i++) {
        while (flow_pid_table[i] != NULL) {
            FLOW_PID_ENTRY *entry = flow_pid_table[i];
            flow_pid_table[i] = entry->next;
            free(entry);
        }
    }
    ReleaseSRWLockExclusive(&flow_pid_lock);
...
static DWORD WINAPI flow_tracker(LPVOID arg)
{
    ...
    while (running) {
        WINDIVERT_ADDRESS addr;
        UINT recv_len = 0;
        if (!WinDivertRecv(flow_handle, NULL, 0, &recv_len, &addr)) {
            if (!running || GetLastError() == ERROR_INVALID_HANDLE) break;
            Sleep(10);
            continue;
        }
        if (addr.Event != WINDIVERT_EVENT_FLOW_ESTABLISHED) continue;
        ...
        if (addr.IPv6) {
            ...
            // WinDivert 2.2 stores an IPv4-mapped address in host-order words
            // as {IPv4, 0x0000FFFF, 0, 0}. Convert word [0] to the packet
            // header's network order before matching the complete tuple.
...
static DWORD WINAPI socket_tracker(LPVOID arg)
{
    ...
    while (running) {
        ...
        if (addr.Event == WINDIVERT_EVENT_SOCKET_CONNECT)
            InterlockedIncrement(&socket_connect_event_count);
        else
            InterlockedIncrement(&socket_bind_event_count);
        if (addr.Socket.LocalPort != 0)
...
typedef enum ATTRIBUTION_SOURCE {
    ATTRIBUTION_SOURCE_NONE = 0,
...
static const char *attribution_source_name(ATTRIBUTION_SOURCE source)
{
    switch (source)
    {
        case ATTRIBUTION_SOURCE_FLOW: return "flow";
...
static const char *rule_action_name(RuleAction action)
{
    switch (action)
    {
...
static void log_resolved_attribution(const char *family, BOOL is_udp,
                                     ATTRIBUTION_SOURCE source,
                                     RuleAction action)
{
    LONG sample = InterlockedIncrement(&resolved_attribution_count);
    if (sample <= 32)
        log_message("[ATTR] Resolved; family=%s protocol=%s source=%s action=%s sample=%ld",
...
static RuleAction check_process_rule_v6(const UINT8 src_ip6[16], UINT16 src_port, const UINT8 dest_ip6[16], UINT16 dest_port, BOOL is_udp, DWORD *out_pid, UINT32 *out_proxy_config_id)
{
    ...
    do {
        ...
        if (pid == 0)
        {
            ...
            pid = get_socket_port_pid_v6(is_udp, src_port, dest_ip6,
                                         dest_port, &cross_family);
            if (pid != 0)
                source = cross_family ?
                    ATTRIBUTION_SOURCE_SOCKET_CROSS_FAMILY :
                    ATTRIBUTION_SOURCE_SOCKET;
...
static BOOL get_process_name_from_pid(DWORD pid, char *name, DWORD name_size)
{
    ...
    DWORD path_len = (DWORD)(sizeof(full_path_w) / sizeof(full_path_w[0]));
...
static RuleAction check_process_rule(UINT32 src_ip, UINT16 src_port, UINT32 dest_ip, UINT16 dest_port, BOOL is_udp, DWORD *out_pid, UINT32 *out_proxy_config_id)
{
    ...
    do {
        ...
        {
            BOOL cross_family = FALSE;
            pid = get_socket_port_pid_v4(is_udp, src_port, dest_ip,
                                         dest_port, &cross_family);
            if (pid != 0)
                source = cross_family ?
                    ATTRIBUTION_SOURCE_SOCKET_CROSS_FAMILY :
...
// A missing or stale proxy ID must fail closed; never silently select another route.
static PROXY_CONFIG* find_proxy_config(UINT32 config_id)
{
...
static int socks5_connect_domain(SOCKET s, const char *hostname, UINT16 dest_port, const PROXY_CONFIG *cfg)
{
    ...
    if (use_auth) { buf[1] = 0x02; buf[2] = SOCKS5_AUTH_NONE; buf[3] = 0x02; if (send_all(s, (char*)buf, 4) != 4) return -1; }
    else          { buf[1] = 0x01; buf[2] = SOCKS5_AUTH_NONE;                 if (send_all(s, (char*)buf, 3) != 3) return -1; }

    len = recv_exact(s, buf, 2);
    ...
    if (buf[1] == 0x02)
    {
        ...
        if (send_all(s, (char*)buf, (int)(3 + user_len + pass_len)) != (int)(3 + user_len + pass_len)) return -1;
        len = recv_exact(s, buf, 2);
    ...
    if (send_all(s, (char*)buf, req_len) != req_len) return -1;
    return socks5_read_reply(s, NULL);
}
...
static int socks5_connect(SOCKET s, UINT32 dest_ip, UINT16 dest_port, const PROXY_CONFIG *cfg)
{
    ...
    if (buf[1] == 0x02)  // Username/password required
    {
        ...
        if (send_all(s, (char*)buf, (int)(3 + user_len + pass_len)) != (int)(3 + user_len + pass_len))
        {
...
static int socks5_connect_v6(SOCKET s, const UINT8 dest_ip6[16], UINT16 dest_port, const PROXY_CONFIG *cfg)
{
    ...
    if (buf[1] == 0x02)
    {
        ...
        if (send_all(s, (char*)buf, (int)(3 + ul + pl)) != (int)(3 + ul + pl)) return -1;
        len = recv_exact(s, buf, 2);
        if (len != 2 || buf[0] != 0x01 || buf[1] != 0x00) return -1;
    }
    else if (buf[1] != SOCKS5_AUTH_NONE) return -1;
    ...
    if (send_all(s, (char*)buf, 22) != 22) return -1;
    return socks5_read_reply(s, NULL);
}
            ...
            if (client_sock != INVALID_SOCKET)
            {
                ...
                if (conn_config != NULL)
                {
                    ...
                    if (get_connection_full(client_port, &conn_config->orig_dest_ip, &conn_config->orig_dest_port, &conn_config->proxy_config_id))
                    {
                        ...
                        if (sample <= 16)
                            log_message("[RELAY] Redirect accepted; family=IPv4 sample=%ld", sample);
                        HANDLE conn_thread = CreateThread(NULL, 0, connection_handler, (LPVOID)conn_config, 0, NULL);
                        if (conn_thread != NULL) { CloseHandle(conn_thread); }
                        else { closesocket(client_sock); free(conn_config); }
                    }
                    else {
                        log_message("[RELAY] Redirect mapping unavailable; family=IPv4");
                        closesocket(client_sock); free(conn_config);
                    }
                }
            ...
            if (client_sock6 != INVALID_SOCKET)
            {
                ...
                if (conn_config != NULL)
                {
                    ...
                    if (get_connection_full_v6(client_port, conn_config->orig_dest_ip6, &conn_config->orig_dest_port, &conn_config->proxy_config_id))
                    {
                        LONG sample = InterlockedIncrement(&relay_accept_log_count);
                        if (sample <= 16)
                            log_message("[RELAY] Redirect accepted; family=IPv6 sample=%ld", sample);
                        HANDLE conn_thread = CreateThread(NULL, 0, connection_handler, (LPVOID)conn_config, 0, NULL);
                        if (conn_thread != NULL) { CloseHandle(conn_thread); }
                        else { closesocket(client_sock6); free(conn_config); }
                    }
                    else {
                        log_message("[RELAY] Redirect mapping unavailable; family=IPv6");
                        closesocket(client_sock6); free(conn_config);
                    }
                }
...
static DWORD WINAPI connection_handler(LPVOID arg)
{
    ...
    if (proxy->type == PROXY_TYPE_SOCKS5)
    {
        ...
        if (rc != 0)
        {
            log_message("[RELAY] SOCKS5 handshake failed; family=%s code=%d",
                is_ipv6 ? "IPv6" : "IPv4", rc);
            closesocket(client_sock);
            closesocket(socks_sock);
            return 0;
        }
    ...
    {
        LONG sample = InterlockedIncrement(&relay_upstream_log_count);
        if (sample <= 16)
            log_message("[RELAY] Upstream ready; family=%s type=%s sample=%ld",
                is_ipv6 ? "IPv6" : "IPv4",
                proxy->type == PROXY_TYPE_SOCKS5 ? "SOCKS5" : "HTTP",
                sample);
    }
...
PROXYBRIDGE_API void ProxyBridge_SetFailClosed(BOOL enable)
{
    if (running)
        return;
    g_fail_closed = enable;
}

static void cleanup_flow_pid_cache(void)
{
    ULONGLONG now = GetTickCount64();
    AcquireSRWLockExclusive(&flow_pid_lock);
    for (int i = 0; i < FLOW_PID_BUCKETS; i++) {
        FLOW_PID_ENTRY **cursor = &flow_pid_table[i];
        while (*cursor != NULL) {
            if (now - (*cursor)->updated_at > FLOW_PID_TTL_MS) {
                FLOW_PID_ENTRY *expired = *cursor;
                *cursor = expired->next;
                free(expired);
            } else {
                cursor = &(*cursor)->next;
...
PROXYBRIDGE_API BOOL ProxyBridge_Start(void)
{
    ...
    flow_handle = WinDivertOpen(
        "true", WINDIVERT_LAYER_FLOW, 124,
        WINDIVERT_FLAG_SNIFF | WINDIVERT_FLAG_RECV_ONLY);
    if (flow_handle == INVALID_HANDLE_VALUE)
    {
        log_message("Failed to open WinDivert flow tracker (%lu)", GetLastError());
        running = FALSE;
        return FALSE;
    }
    flow_thread = CreateThread(NULL, 0, flow_tracker, NULL, 0, NULL);
    if (flow_thread == NULL)
    {
        running = FALSE;
