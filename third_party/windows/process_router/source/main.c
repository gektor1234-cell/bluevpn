...
typedef void (*pfnSetFailClosed)(int enable);
...
static pfnSetFailClosed         g_SetFailClosed  = NULL;
...
static bool load_dll(void)
{
    ...
    LOAD_FN(pfnSetFailClosed,         g_SetFailClosed,  "ProxyBridge_SetFailClosed");
...
int main(int argc, char* argv[])
{
    ...
    for (int i = 1; i < argc; i++)
    {
        ...
        else if (strcmp(argv[i], "--profile") == 0)
        {
            ...
            const char* supplied_profile = argv[++i];
            if (strnlen_s(supplied_profile, MAX_PATH) >= MAX_PATH)
            {
                fprintf(stderr, "ERROR: --profile path exceeds the supported length.\n");
                return 1;
            }
            strcpy_s(profile_path, MAX_PATH, supplied_profile);
        }
    ...
    {
        fprintf(stderr, "ERROR: Profile contains no proxy configuration.\n");
        return 1;
    }
    ...
    for (int i = 0; i < prof.num_rules; i++)
    {
        ...
        if (!r->is_enabled || r->action != 0)
            continue;

        bool found = false;
        for (int j = 0; j < prof.num_configs; j++)
        {
            if (prof.configs[j].profile_id == r->proxy_config_id)
            {
                found = true;
                break;
            }
        ...
        {
            fprintf(stderr,
                "ERROR: Enabled proxy rule %d references unknown proxy config id %u.\n",
                i + 1, r->proxy_config_id);
            return 1;
        }
    }
    ...
    for (int i = 0; i < prof.num_rules; i++)
    {
        ...
        if (r->action == 0)
        {
            ...
            {
                fprintf(stderr,
                    "  [%d] ERROR: Proxy config id %u was not loaded.\n",
                    i + 1, r->proxy_config_id);
                fail++;
                continue;
            }
    ...
    if (fail != 0 || ok == 0)
    {
        fprintf(stderr, "ERROR: Refusing to start with incomplete rule configuration.\n");
        FreeLibrary(g_hDll);
        return 1;
    }
